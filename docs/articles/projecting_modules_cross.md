...
In this tutorial, we demonstrate how to use hdWGCNA to project
co-expression modules from a reference to a query dataset in certain
special cases. Specifically, we project modules from a human snRNA-seq
dataset to a human snATAC-seq dataset, a mouse snRNA-seq dataset, and a
mouse
...
In this section, we project snRNA-seq co-expression modules onto the
snATAC-seq dataset from our [Nature Genetics
paper](https://doi.org/10.1038/s41588-021-00894-z). This analysis
requires a fully processed snRNA-seq dataset that we have run hdWGCNA
on, and a fully processed snATAC-seq dataset. For snATAC-seq processing
and data formatting, we suggest using
[Signac](https://satijalab.org/signac/), an extension of Seurat for
performing single-cell chromatin accessibility analysis. Signac offers
several
[tutorials](https://satijalab.org/signac/articles/mouse_brain_vignette.html)
for processing snATAC-seq data from scratch, and before projecting
co-expression modules into snATAC-seq data we need to do the following
analysis steps:

- [Quality control and
  filtering](https://satijalab.org/signac/articles/pbmc_vignette.html#computing-qc-metrics).
- Term frequency inverse document frequency (TF-IDF) normalization
  ([RunTFIDF](https://satijalab.org/signac/reference/runtfidf)).
- Feature Selection
  ([FindTopFeatures](https://satijalab.org/signac/reference/findtopfeatures)).
- Linear dimensionality reduction
  ([RunSVD](https://satijalab.org/signac/reference/runsvd)).
...
- Create a gene activity matrix
  ([GeneActivity](https://satijalab.org/signac/reference/geneactivity)).
...
To ensure that we loaded the dataset, and that this is indeed a
chromatin dataset, we plot the snATAC-seq UMAP and a chromatin
accessibility coverage plot for *GAD2*, an inhibitory neuron marker
gene.
...
In previous examples, the reference and query datasets were both
transcriptomics datasets, but here we are trying to investigate
co-expression modules in the epigenome using chromatin accessibility
data. Here we use the `ProjectModules` function as we have done before,
but we have to ensure that we are using the Gene Activity matrix from
the snATAC-seq dataset rather than the chromatin accessibility matrix.
...
Using the gene activity matrix we computed module eigengenes in the
snATAC-seq dataset, harmonizing based on the sequencing batch.
Additionally, we computed module hub gene expression scores for the
projected modules, and now we are ready for visualization and downstream
analysis.

### Visualize modules in snATAC-seq

Here we will visualize the hub gene expression scores of the projected
modules in the snATAC-seq dataset using `ModuleFeaturePlot` and Seurat’s
`DotPlot` function.
...
![](figures/projection/atac_projected_featureplots_scores.png)
...
![](figures/projection/atac_projected_scores_dotplot.png)

### Module preservation in snATAC-seq

In this section, we compute the module preservation statistics for the
snATAC-seq dataset. This analysis will tell us which of the
co-expression modules are conserved at the epigenome level, and
specifically which network properties, such as density or connectiviy,
are preserved across modalities. This code is very similar to our
previous module preservation analysis, and we do not have to take any
specific steps for snATAC-seq here. Note that this code can take a few
hours to run on a large query dataset.
...
Now we visualize the results using the `PlotModulePreservation`
function.
...
![](figures/projection/atac_module_preservation_summary.png)
...
![](figures/projection/atac_module_preservation_all.png)
...
![](figures/projection/mouse_umap.png)
...
![](figures/projection/mouse_featureplot.png)
...
![](figures/projection/mouse_dotplot.png)
...
![](figures/projection/mouse_preservation_summary.png)
...
![](figures/projection/visium_preservation_all.png)
...
![](figures/projection/vis_umap.png)
...
![](figures/projection/visium_featureplot.png)
...
![](figures/projection/visium_featureplot_umap.png)
...
![](figures/projection/visium_preservation_summary.png)

...
# compute target gene scores for all TFs, and make it it's own assay in Seurat obj

# this part is hella slow but it keeps seg faulting when I try to parallelize
# might be simpler to try not to collapse in cases where there are multiple
# promoters listed for the same gene...

# test <- do.call(rbind, furrr::future_map(gene_list, function(cur_gene){
test <- do.call(rbind, lapply(1:length(gene_list), function(i){
  cur_gene <- gene_list[i]

  if(i%%1000 == 0){
    print(i)
  }

  cur_rows <- which(rownames(x) == cur_gene)
  cur_tf_matches <- x[cur_rows,]

  if(is.null(dim(cur_tf_matches))){
    cur_tf_matches <- as.numeric(cur_tf_matches)
  } else{
    cur_tf_matches <- colSums(cur_tf_matches)
  }
  cur_tf_matches
}))

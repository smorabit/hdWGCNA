...
pdf(paste0(fig_dir, 'test_overlap_barplot.pdf'), width=12, height=5, useDingbats=FALSE)
...
# traits to correlate with, should be cols in seurat metadata:
traits <- c('pct_counts_mt', 'total_counts')

trait_df <- cur_seurat@meta.data[,traits]

# correlation matrix, but how to get p-val?
cor_mat <- cor(as.matrix(trait_df), as.matrix(MEs))
cor_p  <- corPvalueStudent(cor_mat, ncol(seurat_obj))

# compute correlation matrix separately for each group:


plot_df <- reshape2::melt(cor_mat)

p <- ggplot(plot_df, aes(x=Var2, y=Var1, fill=value)) +
  geom_tile() + scale_fill_gradient2(limits=c(-1,1)) +
  RotatedAxis()



pdf(paste0(fig_dir, 'test_ME_Trait_correlation.pdf'), width=8, height=3)
print(p)
dev.off()
...
TF target scoring function

allow user to select UCell or Seurat

```{r eval=FALSE}

# this is probably gonna take a long time to run
cur_seurat <- MotifTargetScore(
  cur_seurat,
  method='UCell'
)


head(GetMotifScores(cur_seurat))
...
# get TF scores:
tf_scores <- GetMotifScores(cur_seurat)

# correlation matrix:
cor_mat <- cor(as.matrix(MEs), as.matrix(tf_scores))

# melt the correlation matrix
cor_df <- reshape2::melt(cor_mat)
colnames(cor_df) <- c('module', 'motif', 'cor')

# add module color:
mod_colors <- modules %>% dplyr::select(c(module, color)) %>% distinct
cor_df$color <- mod_colors$color[match( cor_df$module, mod_colors$module)]
...
for(cur_mod in mods){
  ...
  print(cur_mod)
  plot_df <- subset(cor_df, module == cur_mod)
  plot_df$motif <- as.character(plot_df$motif)

  # label the top & bottom 5
  labeled <- plot_df %>% top_n(5, cor) %>% .$motif
  labeled <- c(labeled, (plot_df %>% top_n(-5, cor) %>% .$motif))

  plot_df$anno <- ifelse(
    plot_df$motif %in% labeled,
    plot_df$motif,
    ''
  )
  ...
  plot_list[[cur_mod]] <- plot_df %>%
    ggplot(aes(x=reorder(motif, cor), y=cor, fill=cor)) +
    geom_col(width=1) +
    ...
    ylab(paste0('Correlation of Motif Target Scores with ', cur_mod))


}

pdf(paste0(fig_dir, 'test_ME_TF_correlation.pdf'), width=8, height=5)
for(p in plot_list){
  print(p)
}
dev.off()

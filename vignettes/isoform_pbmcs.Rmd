...
two replicates using wget. These counts matrices were generated using the PacBio SMRT Link software, more details can be found at the [isoseq documentation website](https://isoseq.how/umi/).
...
You can also [download the unprocessed Seurat object containing both replicates](https://drive.google.com/file/d/1pIINwgMlJX-3db4sLApt5Gi7GkWQElGf/view?usp=share_link)
...
were detected in both replicates. If you don't want to run this section, you can 
[download the unprocessed Seurat object here](https://drive.google.com/file/d/1pIINwgMlJX-3db4sLApt5Gi7GkWQElGf/view?usp=share_link).
...
major_list2 <- FindMajorIsoforms(
  ...
  replicate_col = 'Sample',
  isoform_delim = '[.]',
  cluster_markers = cluster_markers %>% group_by(cluster) %>% slice_max(n=5, order_by = avg_log2FC)
)
major_marker_list2 <- major_list2[[2]]
major_list2 <- major_list2[[1]]

# major isoform heatmap??
selected_isoforms <- unique(unlist(major_marker_list2))
length(selected_isoforms)
...
png(paste0(fig_dir, 'major_iso_heatmap2.png'), width=8, height=6, units='in', res=500)
p
dev.off()

# <img src="figures/isoseq/major_iso_heatmap.png" width="900" height="600">
...
seurat_obj <- SetupForWGCNA(
  seurat_obj,
  gene_select = "custom",
  gene_list = major_marker_list[['CD14+ Monocyte']],
  metacell_location = 'variable', # this will use the same Metacell seurat object that we made before
  wgcna_name = "major_mono"
)
...
png(paste0(fig_dir, 'major_mono_test_softpower.png'), width=12, height=9, units='in', res=500)
wrap_plots(plot_list, ncol=2)
dev.off()


png(paste0(fig_dir, "major_mono_dendro.png"),height=3, width=6, units='in', res=500)
PlotDendrogram(seurat_obj, main='hdWGCNA Dendrogram')
dev.off()

# <img src="figures/isoseq/major_NK_dendro.png" width="900" height="600">
...
png(paste0(fig_dir, 'major_mono_dotplot_MEs.png'), width=8, height=4, units='in', res=500)
p
dev.off()
...
png(paste0(fig_dir, 'major_mono_vlnplot_MEs.png'), width=7, height=6, res=500, units='in')
p
dev.off()
...
## Example 3: consensus wgcna between the two samples

Coming soon!


# Isoform plotting with ggtranscript and Swan

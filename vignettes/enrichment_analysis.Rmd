...
description: >
  Tutorial for performing enrichment analysis with scWGCNA.
vignette: >
...
In this tutorial, we will perform enrichment tests on our scWGCNA modules.
...
processes that our scWGCNA modules are involved in. Additionally, we perform a
gene set overlap analysis to compare the genes in scWGCNA modules with the marker
...
to visualize the results using scWGCNA. scWGCNA includes the function `RunEnrichr`
...
We store the results of the enrichment tests in the scWGCNA experiment, so can
...
* `module`: the name of the scWGCNA module.
...
scWGCNA includes the function `EnrichrBarPlot` to summarize the results of every
...
scWGCNA includes an additional visualization function for enrichment results,
...
In this section we discuss how to compare scWGCNA modules with cluster or cell-type
marker genes. First we use the Seurat function `FindAllMarkers` to identify the
marker genes in each cell type, and then we use the scWGCNA function `OverlapModulesDEGs`
...
scWGCNA experiment. This is partially because this test is extremely fast to run,
...
scWGCNA includes two functions to visualize the results from `OverlapModulesDEGs`,
...
![](figures/enrichment/overlap_barplot.png)
...
![](figures/enrichment/overlap_dotplot.png)

Each dot is colored by the scWGCNA module's unique color, and the size of the
